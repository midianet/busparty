...
public class BotSession {
    ...
    public BotSession(String token, ITelegramLongPollingBot callback, BotOptions options) {
        ...
        if (options.hasProxy()) {
            ...
            final HttpHost proxy = new HttpHost(options.getProxyHost(), options.getProxyPort());
            credsProvider.setCredentials(new AuthScope(options.getProxyHost(), options.getProxyPort()), new UsernamePasswordCredentials(options.getProxyUser(), options.getProxyPasswd()));
